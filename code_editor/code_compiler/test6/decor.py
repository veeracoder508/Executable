...
def load_screen_reboot() -> None:
    for i in ["▓","▓▓","▓▓▓","▓▓▓▓","▓▓▓▓▓"]:
        clear()
        print(Fore.LIGHTBLACK_EX + f"rebooting[{i:<5}]" + Style.RESET_ALL)
        time.sleep(len(i)/2)
...
def load_screen_load_data() -> None:
    for i in ["▓","▓▓","▓▓▓","▓▓▓▓","▓▓▓▓▓"]:
        ...
        print(Fore.LIGHTBLACK_EX + f"rebooting succussful!" + Style.RESET_ALL)
        print(Fore.LIGHTBLACK_EX + f"reloading[{i:<5}]" + Style.RESET_ALL)
        time.sleep(len(i)/(2*2))
    print(Fore.LIGHTBLACK_EX + f"reloading succussful!" + Style.RESET_ALL)
    clear()
    print(Fore.LIGHTBLACK_EX + f"rebooting[█████]" + Style.RESET_ALL)
    print(Fore.LIGHTBLACK_EX + f"rebooting succussful!" + Style.RESET_ALL)
    print(Fore.LIGHTBLACK_EX + f"reloading[█████]" + Style.RESET_ALL)
    time.sleep(1)
    
def exit_program():
    for i in ["▓","▓▓","▓▓▓","▓▓▓▓","▓▓▓▓▓"]:
        clear()
        print(Fore.LIGHTBLACK_EX + f"exiting[{i:<5}]" + Style.RESET_ALL)    
        time.sleep(len(i)/(2*2))
    print(Fore.LIGHTBLACK_EX + f"exiting succussful!" + Style.RESET_ALL)
    time.sleep(1)
    clear()